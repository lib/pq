...
func TestUUIDRoundTrip(t *testing.T) {
	...
	var data string
	var expected = "0302fd02-1990-44af-84aa-033595a340a9"

	rows, err := db.Query(`SELECT $1::uuid`, expected)
	if err != nil {
		t.Fatal(err)
	}
	defer rows.Close()
	...
	if data != expected {
		t.Errorf("Expected %q from query, got %q", expected, data)
	}

	stmt, err := db.Prepare(`SELECT $1::uuid`)
	if err != nil {
		t.Fatal(err)
	}
	defer stmt.Close()
	rows, err = stmt.Query(expected)
	if err != nil {
		t.Fatal(err)
	}
	defer rows.Close()
	...
	if data != expected {
		t.Errorf("Expected %q from prepared statement, got %q", expected, data)
	}
}
